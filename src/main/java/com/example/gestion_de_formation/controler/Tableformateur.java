...
public class Tableformateur  implements Initializable{
    ...
    @FXML
    private TableColumn<Viewadmin, String> Email;

    @FXML
    private TableView<?> Formateur;
   ...
   show("Admin","Home ");
    ...
    public void setdata() throws ClassNotFoundException, SQLException{
        DbConnection conn = new DbConnection();
            data.clear();
            String req="SELECT `formateur`.`id`, `formateur`.`nom`, `formateur`.`prenom`, `formateur`.`email`, `domaine`.`Libelle`FROM `formateur` LEFT JOIN `domaine` ON `formateur`.`domaine` = `domaine`.`idDomaine`;";
    ...
    public void settable() throws ClassNotFoundException, SQLException{
        ...
        Nom.setCellValueFactory(new PropertyValueFactory<Viewadmin,String>("item2"));
        Prenom.setCellValueFactory(new PropertyValueFactory<Viewadmin,String>("item3"));
        Email.setCellValueFactory(new PropertyValueFactory<Viewadmin,String>("item4"));
        Formation.setCellValueFactory(new PropertyValueFactory<Viewadmin,String>("item4"));
        table.getColumns().addAll(Id,Nom,Prenom,Email,Formation);
